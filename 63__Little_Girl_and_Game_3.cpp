...
bool Can_be_a_Palindrome(VI &Fre){
  int ok=0;
  fo(i,0,25)if(Fre[i]&1)ok++;
  return ok<=1;
}
...
void solve(){//File();
  ...
  cin >> S;
  int N=S.length();
  string FI="First",SE="Second";
  fo(G,1,N){
    VI Fre(26,0); for(auto I:S)Fre[I-'a']++;
    if(Can_be_a_Palindrome(Fre)){
      if(G&1)cout << FI;
      else cout << SE;
      return ;
    }
      ...
      string NS="";
      bool Made_even=false;
      fo(i,0,25){
        if(!Made_even && Fre[i]&1){
          Made_even=true;
          Fre[i]--;
        }
        fo(j,1,Fre[i])NS+=(char('a'+i));
      }
      S=NS;
    }
  }
