...
ll NCR(ll n,ll r){if(n<r)return 0;if(n==r)return 1;if(r==0)return 1;ll ans = 1;for(ll i=1;i<=n;i++)ans = ((ans%MOD) * (i%MOD) )%MOD;for(ll i=1;i<=r;i++)ans = ((ans%MOD) * (ModInverse(i)))%MOD;for(ll i=1;i<=n-r;i++)ans = ((ans%MOD) * (ModInverse(i)))%MOD;ans%=MOD;return ans;}
