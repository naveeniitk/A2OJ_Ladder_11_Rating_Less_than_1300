...
void solve(){//File();
  ...
  fo(i,1,T){
    if(Wind[i-1]=='E' && Sx<Ex)Sx++;
    else if(Wind[i-1]=='W'&&Sx>Ex)Sx--;
    else if(Wind[i-1]=='N' && Sy<Ey)Sy++;
    else if(Wind[i-1]=='S' && Sy>Ey)Sy--;
    if(Sx==Ex && Sy==Ey){
      cout << i; return;
    }
  }
...
// ------------------------------------------------------------------------------
  ...
  // ll Len=0,Ans=0;
  // fo(i,1,T){
  //   if(Wind[i-1]==WD_Take[Len])Len++; 
  //   if(Len==WD_Take.length()){
  //     Ans=i;break;
  //   }
  // }
  // if(Ans==0)cout << -1 ;
